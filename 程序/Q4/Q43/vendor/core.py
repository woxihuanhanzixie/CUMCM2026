...
ADJ_RATE=0.5   # adjustment surcharge coefficient (symmetric |a-q0|); frozen = 0.5
EMG_MULT=5.0   # emergency price multiple; frozen = 5
def serial(x):
...
def solve_lp(p,budget,label):
    ...
    kappa=np.zeros(n); kappa[:p.today]=ADJ_RATE if p.stage!='original' else 0.
    q=np.r_[p.q0,np.zeros(n-p.today)]; fee=p.prices*(A+kappa*abs(A-q)+EMG_MULT*g)
...
def reference_milp(p,budget,label):
    ...
    for t in range(n):
        ...
        cost[a]=p.prices[t]; cost[g]=EMG_MULT*p.prices[t]; cost[d]=ADJ_RATE*p.prices[t] if adj else 0
