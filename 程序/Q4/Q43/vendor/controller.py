...
import numpy as np
from core import Problem,solve_lp,execute,LO,digest,ADJ_RATE,EMG_MULT
...
class Controller:
    ...
    def step(self):
        ...
        row.update(normal=price*A,adjustment=ADJ_RATE*price*abs(A-q),emergency=EMG_MULT*price*act['g']); row['total']=row['normal']+row['adjustment']+row['emergency']
