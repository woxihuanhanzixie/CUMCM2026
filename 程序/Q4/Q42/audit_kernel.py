...
def audit(bundle,raw,expected_segments=144,initial_energy=10800.):
    ...
    E=initial_energy; max_phys=max_bill=max_price=max_obj=0.; bills=[]; failures=0; unproven=0; secondary=0
    ...
    for plan in plans:
        ...
        expected_length=144 if name=='M0' else min(288-t,52560-i)
    ...
    return dict(status='PASS',segments=len(rows),plans=len(plans),normal=math.fsum(r['normal'] for r in rows),adjustment=math.fsum(r['adjustment'] for r in rows),emergency=math.fsum(r['emergency'] for r in rows),total=math.fsum(bills),initial_energy=initial_energy,final_energy=E,max_physical_residual=max_phys,max_bill_error=max_bill,max_price_rebuild_error=max_price,max_plan_objective_error=max_obj,fallbacks=failures,unproven=unproven,secondary_accepted=secondary,numerical_zero_events=len(bundle['corrections']))
